...
def generel_expand(filename : str, states : list[int]):
    res = []
    first_state, last_state, offset = states
    ...
    for line in lines:
        if "#load" in line:
            ...
            if (linesplit[0] == "(1"):
                linesplit[0] = first_state
            else:
                linesplit[0] = str(int(first_state_rule) + int(first_state)-1 + int(offset))
            if (last_state_rule == "0"):
                linesplit[-1] = last_state
            else:
                linesplit[-1] = str(int(last_state_rule) + int(first_state)-1 + int(offset))
...
def condititional_expand(filename : str, states : list[int]):
    res = []
    first_state, true_state, false_state, offset = states
    file = open("Compiler/macros/" + filename + ".txt")
    lines = file.readlines()
    for line in lines:
        if "#load" in line:
